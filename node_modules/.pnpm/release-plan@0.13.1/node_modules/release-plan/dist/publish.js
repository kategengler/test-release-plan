...
export async function createGithubRelease(octokit, description, tagName, reporter, options) {
    try {
...
export async function npmPublish(solution, reporter, options, packageManager) {
    ...
    for (const [pkgName, entry] of solution) {
        ...
        if (preExisting) {
            info(`${pkgName} has already been published @ version ${entry.newVersion}. Skipping publish;`);
            continue;
        }
        if (options.dryRun) {
            info(`--dryRun active. Adding \`--dry-run\` flag to \`${packageManager} publish${options.otp ? ' --otp=*redacted*' : ''}\` for ${pkgName}, which would publish version ${entry.newVersion}\n`);
